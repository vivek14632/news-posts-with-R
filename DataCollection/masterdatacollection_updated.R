  ...
  browser()
	...
	if(length(fb_page$id)<1)
	{
  	...
  	for(value in 1:length(fb_page$id))
  	{
    		...
    		#browser()
		tryCatch({
    			posts[[value]] <- getPost(fb_page$id[value], toString(fb_oauth), comments = TRUE, likes = TRUE, reactions = TRUE,
                       n.likes = fb_page$likes_count[value], n.comments = fb_page$comments_count[value])
    			shares[[value]] <- getShares(post=fb_page$id[value], fb_page$shares_count[value], token=toString(fb_oauth))
		},warning=function(w){
			print(w)
		},error=function(e){
			print(e)
		...
		if(posts[[i]]$post$comments_count>0)
			...
			temp=rep(0,nrow(posts[[i]]$comments))
        		...
        		print(paste('comment count=',posts[[i]]$post$comments_count))
      			for(j in 1:posts[[i]]$post$comments_count)
      			{
